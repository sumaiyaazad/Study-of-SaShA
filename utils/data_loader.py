...
def load_data_SmallLibraryThing(split=False):

    # Load ratings data
    data = pd.read_csv('data/SmallLibraryThing/ratings.csv', header=None)
    data.columns = ['user_id', 'item_id', 'rating']
...
def load_kg_SmallLibraryThing(items, feature_type='ontological'):
    """
    Load KG data
    params:
        items: items dataframe
    ...
    selected_features = pd.read_csv('data/SmallLibraryThing/selected_features.csv')
    selected_features = literal_eval(selected_features.loc[selected_features['type'] == feature_type]['features'].to_list()[0])

    kg = pd.read_csv('data/SmallLibraryThing/df_map.csv')
    kg.columns = ['feature', 'item_id', 'item_index_discard', 'value']
